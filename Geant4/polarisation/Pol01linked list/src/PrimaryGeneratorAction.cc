...
: G4VUserPrimaryGeneratorAction(), 
  fParticleGun(0), fDetector(det) ,fInput(inp),checkValue(0)                                 
{
...
//  G4int divisor = 100000;
//  if(nOfEvent!=0&&nOfEvent%divisor==0){fInput->RefreshData();}
  fInput->Changenode();
  G4ThreeVector sPosition = fInput->PositionChoose();
  fParticleGun->SetParticlePosition(sPosition);
  G4double Energy = fInput->Choose(); 
