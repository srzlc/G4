                         ...
                         G4double longitudinalPolarization)
{
  ...
  if (particle == fGamma) { 
    ...
    if(gamma.is_open())
    {
      gamma<<kinEnergy/MeV<<"    "<<costheta<<"    "<<phi<<"    "<<xPolarization<<"    "<<yPolarization<<"    "<<longitudinalPolarization<<G4endl;
    }
    gamma.close();
  } 
  else if (particle == fElectron) { 
    fElectronStats.FillData(kinEnergy, costheta, longitudinalPolarization);
    std::ofstream electron;
    electron.open("Electron.txt",std::ios::app);
    if(electron.is_open())
    {
    electron<<kinEnergy/MeV<<"    "<<costheta<<"    "<<phi<<"    "<<xPolarization<<"    "<<yPolarization<<"    "<<longitudinalPolarization<<G4endl;
    }
    electron.close();
  } 
  else if (particle == fPositron) {
    fPositronStats.FillData(kinEnergy, costheta, longitudinalPolarization);
    std::ofstream positron;
    positron.open("Positron.txt",std::ios::app);
    if(positron.is_open())
    {
    positron<<kinEnergy/MeV<<"    "<<costheta<<"    "<<phi<<"    "<<xPolarization<<"    "<<yPolarization<<"    "<<longitudinalPolarization<<G4endl;
    }
    positron.close();
  }
