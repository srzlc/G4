...
//set target figure
  G4double shape_rmin=0.*cm,shape_rmax=(0.5+0.1*(copy-i))*cm;
...
//set detector figure
  G4double shape2_PX   = 5.*cm,shape2_PY=5.*cm;
  G4double shape2_PZ   = 5.*cm;
//solid
  G4Box* solidShape2=
    new G4Box("SShape2",
                 shape2_PX,shape2_PY,shape2_PZ);
...
//  logicShape2->SetVisAttributes(G4VisAttributes::GetInvisible());
  return physWorld;
}
...
//sensitive detector
  ...
  pr = new G4PSFlatSurfaceCurrent("NofPositron",1);
  pr->SetFilter(positronFilter);
  det->RegisterPrimitive(pr);
