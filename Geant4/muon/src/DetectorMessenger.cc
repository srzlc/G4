...
:G4UImessenger(), 
 fDetector(det), fTestemDir(0), fDetDir(0),fTarDir(0),
 fMaterCmd(0), fSizeXYCmd(0), fSizeZCmd(0), fUpdateCmd(0),fRMinCmd(0)
{ 
  fTestemDir = new G4UIdirectory("/testem/");
  fTestemDir->SetGuidance("commands specific to this example");
  
  fTarDir = new G4UIdirectory("/testem/tar/");
  fTarDir->SetGuidance("target construction");
        
  fMaterCmd = new G4UIcmdWithAString("/testem/tar/setMat",this);
  fMaterCmd->SetGuidance("Select material of the target");
  fMaterCmd->SetParameterName("choice",false);
  fMaterCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
  
  fSizeXYCmd = new G4UIcmdWithADoubleAndUnit("/testem/tar/setSizeXY",this);
  fSizeXYCmd->SetGuidance("Set sizeXY of the target");
  ...
  fSizeZCmd = new G4UIcmdWithADoubleAndUnit("/testem/tar/setSizeZ",this);
  fSizeZCmd->SetGuidance("Set sizeZ of the target");
  ...
  fRMinCmd = new G4UIcmdWithADoubleAndUnit("/testem/det/setRMin",this);
  fRMinCmd->SetGuidance("Set RMin of the Detector");
  fRMinCmd->SetParameterName("RMin",false);
  fRMinCmd->SetRange("RMin>0.");
...
{ 
  ...
  if( command == fRMinCmd )
   { fDetector->SetRMin(fRMinCmd->GetNewDoubleValue(newValue));
     fDetector->SetRMax(fRMinCmd->GetNewDoubleValue(newValue));}     
