...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  sSphere = new G4Sphere("Detector",                              //its name
                         fRMin,fRMax,pSPhi,pDPhi,pSTheta,pDTheta);//its size
  G4LogicalVolume*
...
{
  G4cout << "\n The Targrt is " << G4BestUnit(fBoxSizeXY,"Length")
         << " x " << G4BestUnit(fBoxSizeXY,"Length")
         << " x " << G4BestUnit(fBoxSizeZ,"Length")
         << " of " << fTargetMaterial->GetName() << G4endl;
  G4cout << "==================================="<< G4endl;
  G4cout << " The Detector is " << G4BestUnit(fRMin,"Length")
         << " Inside Radius "<< G4BestUnit(fRMax,"Length")
         << " Outside Radius "<<"of "<< fWorldMaterial->GetName()<<G4endl;
}
...
void DetectorConstruction::SetRMin(G4double value)
{
  fRMin = value;
  if (fWorldSize<fRMin) fWorldSize = 2.*fRMin;
//  UpdateGeometry();
}
void DetectorConstruction::SetRMax(G4double value)
{
  fRMax = (value/cm+2.)*cm;
  if (fWorldSize<fRMax) fWorldSize = 2.*fRMax;
  UpdateGeometry();
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
